...
// v2: [decodeBlockToByte] customExp instead of big.Exp
func (rsa *RSA) decodeBlockToByte(c []byte) byte {
	cBig := new(big.Int).SetBytes(c)
	mBig := customExp(cBig, rsa.d, rsa.n)
	m := mBig.Bytes()
	if len(m) == 0 {
		return 0
	}
